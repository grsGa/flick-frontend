...
const FollowersClient: React.FC<FollowersClientProps> = ({
  ...
  const { followers, pageInfo, fetchMore, loading, refetch } = useFollowers(userId);
  const hasRefetched = useRef(false);

  console.log('[FollowersClient] Component rendered:', {
    currentUser: !!currentUser,
    loading,
    hasRefetched: hasRefetched.current,
  ...
  useEffect(() => {
    if (currentUser && !loading && !hasRefetched.current) {
      console.log('[FollowersClient] User authenticated, refetching once to get correct follow states');
      hasRefetched.current = true;
      refetch({
        fetchPolicy: 'network-only' // Force network request to get fresh data
      }).then((result) => {
        console.log('[FollowersClient] Refetch completed, new data:', result.data);
        const followersUsers = result.data?.followers?.edges?.map(edge => ({
          username: edge.node.username,
          isFollowing: edge.node.isFollowing
        }));
        console.log('[FollowersClient] Followers users:', followersUsers);
        
        // Log detailed comparison
        followersUsers?.forEach(user => {
          console.log(`[FollowersClient] User ${user.username}: isFollowing = ${user.isFollowing}`);
        });
        console.log('[FollowersClient] Updated followers data with correct follow states');
      }).catch((error) => {
        console.error('[FollowersClient] Refetch failed:', error);
  ...
  const items = currentUser && followers !== undefined ? followers : 
                !currentUser && initialFollowers.length > 0 ? initialFollowers :
                followers !== undefined ? followers : initialFollowers;
    ...
    return (
      <div className="bg-white">
        <div className="text-center p-12">
          ...
          <p className="text-gray-500 max-w-sm mx-auto">When someone follows this account, they'll show up here.</p>
        </div>
      </div>
    );
  }
